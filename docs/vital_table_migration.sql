-- Migration script to add all 22 vital metric fields to the vital table
-- Run this script to update the existing vital table structure
...
-- Add Clinical Vitals Fields (rename existing temp to temperature)
...
-- Update existing data (copy temp to temperature)
UPDATE vital SET temperature = temp WHERE temp IS NOT NULL;

-- Drop old temp column (optional - uncomment if you want to remove it)
-- ALTER TABLE vital DROP COLUMN temp;

-- Drop old hr column (optional - uncomment if you want to remove it)  
